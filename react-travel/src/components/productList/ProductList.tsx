...
const listData = (productList: Product[]) =>
  productList.map((p) => ({
    ...
    tags: (
      <>
        {p.departureCity && <Tag color="#f50">{p.departureCity}</Tag>}
        {p.travelDays && <Tag color="#108ee9">{p.travelDays} days </Tag>}
        {p.discountPresent && <Tag color="#87d068">Super Discount</Tag>}
...
export const ProductList: React.FC<PropsType> = ({
  ...
  return (
    <List
      ...
      footer={
        <div>
          Number of Results: <Text strong>{paging.totalCount}</Text>
        </div>
      }
      renderItem={(item) => (
        <List.Item
