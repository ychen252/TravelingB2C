...
interface FetchRecommendStartAction {
  type: typeof FETCH_RECOMMEND_START;
}
interface FetchRecommendSuccessAction {
  type: typeof FETCH_RECOMMEND_SUCCESS;
  payload: any;
}
interface FetchRecommendFailAction {
  type: typeof FETCH_RECOMMEND_FAIL;
  payload: any;
}

export type RecommendAction =
  | FetchRecommendStartAction
  | FetchRecommendSuccessAction
  | FetchRecommendFailAction;

export const fetchRecommendationStartActionCreator =
  (): FetchRecommendStartAction => {
    return { type: FETCH_RECOMMEND_START };
  };

export const fetchRecommendationSuccessActionCreator = (
  data
): FetchRecommendSuccessAction => ({
...
export const fetchRecommendationFailActionCreator = (
  error
): FetchRecommendFailAction => ({ type: FETCH_RECOMMEND_FAIL, payload: error });

export const getDataActionCreator =
  (): ThunkAction<void, RootState, unknown, RecommendAction> =>
  async (dispatch, getState) => {
    dispatch(fetchRecommendationStartActionCreator());
    try {
      const { data } = await axios.get(`${API_URL}/api/productCollections`);
